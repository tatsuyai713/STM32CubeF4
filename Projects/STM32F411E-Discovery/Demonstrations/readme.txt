...
LED2 (PWR) light up.
- All 4 LEDs between B1 (User) and B2 (Reset) are blinking as chenillard.
- Press User Button B1 (User) then MEMS sensor (LSM303DLHC for Rev.A boards or
LSM303AGR for Rev.C boards) is enabled, move the board and observe the four Leds
blinking according to the board orientation in space (X and Y axes).
...
- Press B1 (User) or B2 (Reset) to restart demonstration.
